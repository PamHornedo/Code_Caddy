...
function BookCollection() {
  const { books, removeBook, updateBookStatus, getBooksByStatus } =
    useBookCollection();
  const [filterStatus, setFilterStatus] = useState("all");
  const navigate = useNavigate();
  const filteredBooks =
    filterStatus === "all" ? books : getBooksByStatus(filterStatus);
    ...
    <div className="book-collection">
      ...
      {filteredBooks.length === 0 ? (
        <div className="empty-collection">
          <p>
            No books found. Start by searching for books to add to your
            collection!
          </p>
        </div>
      ) : (
